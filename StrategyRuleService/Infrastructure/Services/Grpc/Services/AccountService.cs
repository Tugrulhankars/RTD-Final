...
public class AccountService : IAccountService
{
    ...
    public async Task<double> GetAccountBalanceAsync(int accountId)
    {
        try
        {
            // GRPC client'ı manuel olarak oluştur
...
// Manuel olarak GRPC client class'larını oluştur
public class AccountServiceClient
{
    ...
    {
        // Bu method gerçek GRPC server'a bağlanacak
        // Şimdilik mock response döndür
        await Task.Delay(1); // async uyarısını önlemek için
...
// Message class'larını manuel olarak oluştur
